...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        to_update = Request.objects.filter(status=2, date__lt=date.today())
        for request in to_update:
            update_request_status(request)
        self.stdout.write(self.style.SUCCESS(f'{to_update.count()} requests was checked for valid status.'))
